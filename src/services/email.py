...
async def send_email(email: EmailStr, username: str, host: str):
    ...
    -username: this is used in conjunction with host and token_verification to create a link that will be sent
    via email. This username parameter will be passed into auth_service's create_email_token function, which
    returns a token that can then be verified by auth service's verify_email function (see below). The username
